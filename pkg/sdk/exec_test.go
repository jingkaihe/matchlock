...
func TestExecPipe(t *testing.T) {
	tests := []struct {
		name     string
		exec     func(*Client, io.Reader, io.Writer, io.Writer) (*ExecPipeResult, error)
		wantDir  string
		wantUser string
	}{
		{
			name: "with options",
			exec: func(client *Client, stdin io.Reader, stdout, stderr io.Writer) (*ExecPipeResult, error) {
				return client.ExecPipeWithOptions(context.Background(), "cat", ExecPipeOptions{
					WorkingDir: "/tmp",
					User:       "1000:1000",
					Stdin:      stdin,
					Stdout:     stdout,
		...
		},
		{
			...
			exec: func(client *Client, stdin io.Reader, stdout, stderr io.Writer) (*ExecPipeResult, error) {
				return client.ExecPipe(context.Background(), "cat", stdin, stdout, stderr)
			},
		},
		{
			name: "deprecated working directory method",
			exec: func(client *Client, stdin io.Reader, stdout, stderr io.Writer) (*ExecPipeResult, error) {
				return client.ExecPipeWithDir(context.Background(), "cat", "/tmp", stdin, stdout, stderr)
			},
			wantDir: "/tmp",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := tt.exec(client, stdin, &stdout, &stderr)
			require.NoError(t, err)
